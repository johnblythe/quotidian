...
/**
 * Mark personalization celebration as shown
 * Called after displaying the "Personalization Unlocked" message
 */
export async function markPersonalizationCelebrated(): Promise<void> {
  const existing = await db.preferences.toCollection().first();
  if (existing) {
    await db.preferences.update(existing.id!, {
      personalizationCelebrated: true,
    });
  }
}

/**
 * Check if personalization celebration has been shown
 */
export async function hasPersonalizationCelebrated(): Promise<boolean> {
  const prefs = await getPreferences();
